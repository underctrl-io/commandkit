...
### `useRest` (development version - optional)

-   Type `boolean`

This is used to change the behaviour of how CommandKit loads application commands. By default it's one-by-one while comparing changes. Setting this option to `true` will use @discordjs/rest to load application commands all at once on every restart, and when [reloadCommands()](/typedef/CommandKit#reloadcommands-development-version) is called.
