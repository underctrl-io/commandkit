...
<GenerationInfo sourceFile="packages/analytics/src/umami/provider.ts" sourceLine="13" packageName="@commandkit/analytics" />

Umami analytics provider for CommandKit.
This provider allows you to track events and identify users using Umami.

```ts title="Signature"
class UmamiProvider implements AnalyticsProvider {
...
<MemberInfo kind="method" type={`(umami: Umami) => UmamiProvider`}   />
