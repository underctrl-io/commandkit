...
## Example: Setting Up a Cache Provider
...
onApplicationBootstrap(async (commandkit) => {
  commandkit.setCacheProvider(new RedisCache({...}));
});
```

:::info
Note that `@commandkit/redis` provides a plugin to register the cache provider automatically. This example is for demonstration purposes only.
:::
