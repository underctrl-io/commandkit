...
CommandKit automatically configures JSX support in your project. When you import CommandKit, it sets up the necessary JSX runtime:

```tsx
import CommandKit from 'commandkit';
```

This import is required as CommandKit uses `CommandKit.createElement` and `CommandKit.Fragment` under the hood to transform JSX into Discord message components.
...
1. Always import CommandKit as the default import:

```tsx
import CommandKit from 'commandkit';
```

2. Use named imports for specific components:

```tsx
import { Button, ActionRow } from 'commandkit';
```

3. Keep component trees readable with proper indentation:
...
5. Create custom components for reusable Discord.js objects:
