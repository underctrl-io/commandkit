...
CommandKit provides a powerful yet simple caching system that works out of the box. This guide will show you how to effectively use caching in your bot.
...
onApplicationBootstrap(async (commandkit) => {
  ...
  commandkit.setCacheProvider(new RedisCache(redis));
});
```
...
```ts
import { ChatInputCommand, CommandData, cacheTag } from 'commandkit';
...
commandkit.setCacheProvider(new MyCustomCache());
```
