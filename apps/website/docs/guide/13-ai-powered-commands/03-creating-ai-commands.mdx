...
The `ai` function behaves similar to a `message` command, but it receives an additional `ctx.ai` property that contains the parameters passed by the AI model.
...
export const ai: MessageCommand = async (ctx) => {
