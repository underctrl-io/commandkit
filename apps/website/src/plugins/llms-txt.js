...
const DOCS_BASE_URL = process.env.DOCS_BASE_URL || 'https://commandkit.dev/docs';
    ...
    return {
        ...
        loadContent: async () => {
            ...
            if (fs.existsSync(versionedDocsDir)) {
                const versionDirs = await fs.promises.readdir(versionedDocsDir, { withFileTypes: true });
                for (const versionDir of versionDirs) {
                    if (versionDir.isDirectory()) {
                        const versionName = versionDir.name.replace('version-', '');
                        await getMdxFiles(path.join(versionedDocsDir, versionDir.name), versionName);
                    }
                }
            }
